...
ProfileManager = LibStub("AceAddon-3.0"):NewAddon("ProfileManager","AceConsole-3.0", "AceEvent-3.0")
...
local function serialize(data,header)
    ...
    local data = Base64.Encode(data)
    return data 
end

local function unserialize(str)
    str = Base64.Decode(str)
    local success, obj = Serpent.fns.load(str) 
...
UIPanelWindows["BindPadImportExport"] = { area = "center", pushable = 10, whileDead = 1 };

local BINDPAD_MAXSLOTS_DEFAULT = 100;
...
function BindPadImportExportFrame_Toggle()
    if BindPadImportExport:IsVisible() then
        HideUIPanel(BindPadImportExport);
    else
        ShowUIPanel(BindPadImportExport);
    end
end
...
function BindPad_SlashCmd(msg)
    ...
    elseif cmd == "export" then
        ProfileManager:OpenWindow("export");
    elseif cmd == "import" then
        ProfileManager:OpenWindow()
    else
...
function table.serialize(tbl)
    ...
    for k, v in pairs(tbl) do
        ...
        local value = type(v) == "table" and table.serialize(v) or type(v) == "string" and string.format("%q", v) or v
        str = str..key.."="..value..","
    end
    return str.."}"
end
...
function BindPadCore.DoExport()
    local profile = BindPadVars[BindPadCore.character]
    if profile then
        local data = profile
...
function BindPadCore.DoImport(data)
    local decoded = unserialize(data)
    ...
    BindPadVars[BindPadCore.character] = decoded
    BindPadCore.InitProfile();
    if BindPadFrame:IsShown() then
        BindPadFrame_OnShow();
    end
...
-- profiles
function ProfileManager:OpenWindow(input)
...
function ProfileManager:CreateCopyDialog(text)
...
function ProfileManager:CreateWindow(generate)
    local frame = AceGUI:Create("Frame")
    frame.frame:SetFrameStrata("BACKGROUND")
    ...
    frame:SetCallback("OnEscapePressed", function(widget)
       widget:Hide() 
    end)

    if generate then
        frame:SetTitle("Export profile")
        frame:SetStatusText("Click 'Generate' to create an import code")
    end

    if not generate then
        frame:SetTitle("Import profile")
        frame:SetStatusText("Paste a profile code and then click 'Import'")
    end
    ...
    if not generate then
        jsonbox:SetLabel("Copy or paste a profile code into the box below")
    else
        jsonbox:SetLabel("Your character's profile code will appear below")
    end
	...
	local function _generate()
        local data = BindPadCore.DoExport()
		jsonbox:SetText(data) 
		jsonbox:HighlightText()
		jsonbox:SetFocus()
		frame:SetStatusText("Profile code "..BindPadCore.character)
	end

    local function _import()
        local raw = jsonbox:GetText()
        local result = BindPadCore.DoImport(raw)
        if result then
            frame:SetStatusText("Profile imported!")
            return true
        end
    end
